...
/*   Updated: 2025/07/30 19:26:37 by rcarpio-cye      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	handle_mutex_error(int status, t_opcode opcode)
{
	...
	if (EINVAL == status && (LOCK == opcode || UNLOCK == opcode))
		...
		error_exit("Error: A deadlock would occur if thread blocked waiting for mutex");
	else if (EPERM == status)
		error_exit("Error: the current thread does not hold a lock on mutex");
	else if(ENOMEM == status)
		error_exit("Error: the process cannot allocate enough memory to create another mutex");
...
static void handle_thread_error(int status, t_opcode opcode)
{
	...
	if (EAGAIN == status)
		...
		error_exit("Error: no thread could be found corresponding to that specified by given thread ID , thread");
	else if (EDEADLK == status)
		error_exit("Error: a deadlock was detected or the value of thread specifies the calling thread");
}
...
void safe_thread(pthread_t *thread, void *(*foo)(void *), void *data, t_opcode opcode)
{
	if (CREATE == opcode)
		...
		error_exit("Error: wrong opcode for thread_handle: use <CREATE> <JOIN> <DETACH>");
}
