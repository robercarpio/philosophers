...
/*   Updated: 2025/08/19 22:18:10 by rcarpio-cye      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	philo_died(t_philo *philo)
{
	...
	if (get_bool(&philo->mutex_philo, &philo->full))
		return (false);
	elapsed = gettime(MILLISECOND) - get_long(&philo->mutex_philo, &philo->last_meal_time);
	time_to_die = philo->table->tt_die / 1e3;
	// time_to_die = philo->table->tt_die;
